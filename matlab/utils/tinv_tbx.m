function val = tinv_tbx(p, df)
%TINV_TBX Inverse of Student's t-distribution CDF (no Statistics Toolbox).
%   VAL = TINV_TBX(P, DF) returns the inverse CDF value for the
...
%Rational approximation of normal inverse CDF (Abramowitz & Stegun)
if p <= 0
    x = -Inf; return;
end
if p >= 1
